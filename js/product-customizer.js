(() => {
  ...
  const state = { step: 1, color: 'white', size: '', side: 'front', mockupLoaded: false, sides: { front: createSideState(), back: createSideState() } };
  ...
  function updatePlaceholder() { if (!els.placeholder) return; els.placeholder.hidden = state.mockupLoaded || Boolean(currentSide().url); }
  function updateMockup() { const source = product.krasas[state.color]?.mockups?.[state.side]; state.mockupLoaded = false; if (!source) { els.shirt.hidden = true; updatePlaceholder(); return; } els.shirt.hidden = false; els.shirt.src = source; els.shirt.alt = `${colorLabel()} T-krekla ${sideLabel().toLowerCase()}s priekšskatījums`; updatePlaceholder(); }
  els.shirt.addEventListener('load', () => { state.mockupLoaded = true; els.shirt.hidden = false; updatePlaceholder(); requestAnimationFrame(renderDesign); });
  els.shirt.addEventListener('error', event => { state.mockupLoaded = false; els.shirt.hidden = true; updatePlaceholder(); console.warn('PrintStich konfigurators: mokapa attēlu neizdevās ielādēt; tiek izmantots 1200×1400 fallback.', event); requestAnimationFrame(renderDesign); });

  function fallbackFrame() {
    const preview = els.preview.getBoundingClientRect();
    if (!preview.width || !preview.height) return null;
    const scale = Math.min(preview.width / FALLBACK_WIDTH, preview.height / FALLBACK_HEIGHT);
    const width = FALLBACK_WIDTH * scale;
    const height = FALLBACK_HEIGHT * scale;
    return { left: (preview.width - width) / 2, top: (preview.height - height) / 2, width, height };
  }
  function updateFallbackPrintArea() {
    if (state.mockupLoaded) { updatePrintArea(); return; }
    const zone = product.drukasZona[state.side], frame = fallbackFrame();
    if (!zone || !frame) return;
    const preview = els.preview.getBoundingClientRect();
    Object.assign(els.printArea.style, {
      left: `${(frame.left + zone.x * frame.width) / preview.width * 100}%`,
      top: `${(frame.top + zone.y * frame.height) / preview.height * 100}%`,
      width: `${zone.w * frame.width / preview.width * 100}%`,
      height: `${zone.h * frame.height / preview.height * 100}%`
    });
  }
  ...
  function loadFile(file){const message=validateFile(file);if(message){error('file',message);return;}error('file');const side=currentSide();if(side.url)URL.revokeObjectURL(side.url);side.file=file;side.url=URL.createObjectURL(file);side.positionX=.5;side.positionY=.5;side.scale=.5;side.preset='center';updatePlaceholder();const image=new Image();image.onload=()=>{side.naturalWidth=image.naturalWidth;side.naturalHeight=image.naturalHeight;els.fileName.textContent=`${sideLabel()}: ${file.name}`;enforcePrintLimit();renderDesign();updateNavigation();};image.onerror=()=>{side.file=null;URL.revokeObjectURL(side.url);side.url='';updatePlaceholder();error('file','Attēlu neizdevās nolasīt. Izvēlies citu failu.');updateNavigation();};image.src=side.url;}
  ...
  els.remove.addEventListener('click',()=>{const side=currentSide();if(side.url)URL.revokeObjectURL(side.url);Object.assign(side,createSideState());els.designInput.value='';els.fileName.textContent='';els.design.hidden=true;updatePlaceholder();error('file');updateMeasurementUi();updateNavigation();if(state.step===3)showStep(2);});
  ...
  function loadImage(source){return new Promise((resolve,reject)=>{if(!source){reject(new Error('Attēla avots nav norādīts.'));return;}const image=new Image();image.onload=()=>resolve(image);image.onerror=()=>reject(new Error(`Neizdevās ielādēt attēlu: ${source}`));image.src=source;});}
  function drawFallbackCanvas(ctx, canvas) { ctx.fillStyle='#efefec';ctx.fillRect(0,0,canvas.width,canvas.height);const zone=product.drukasZona[state.side];const zoneX=zone.x*canvas.width,zoneY=zone.y*canvas.height,zoneW=zone.w*canvas.width,zoneH=zone.h*canvas.height;ctx.save();ctx.strokeStyle='rgba(18,63,55,.55)';ctx.lineWidth=2;ctx.setLineDash([10,8]);ctx.strokeRect(zoneX,zoneY,zoneW,zoneH);ctx.restore();return {zoneX,zoneY,zoneW,zoneH}; }
  async function drawFinalPreview(){const canvas=els.finalPreview;if(!canvas)return;const ctx=canvas.getContext('2d'),mockupSource=product.krasas[state.color]?.mockups?.[state.side],side=currentSide();canvas.width=900;canvas.height=1050;ctx.clearRect(0,0,canvas.width,canvas.height);let zone=drawFallbackCanvas(ctx,canvas);try{try{const shirt=await loadImage(mockupSource);ctx.clearRect(0,0,canvas.width,canvas.height);ctx.drawImage(shirt,0,0,canvas.width,canvas.height);const z=product.drukasZona[state.side];zone={zoneX:z.x*canvas.width,zoneY:z.y*canvas.height,zoneW:z.w*canvas.width,zoneH:z.h*canvas.height};}catch(mockupError){console.warn('PrintStich konfigurators: gala preview mokaps nav pieejams; ģenerēju fallback preview.',mockupError);zone=drawFallbackCanvas(ctx,canvas);}if(!side.url)return;const design=await loadImage(side.url),aspect=design.naturalWidth/design.naturalHeight;let width=zone.zoneW*side.scale,height=width/aspect;if(height>zone.zoneH*side.scale){height=zone.zoneH*side.scale;width=height*aspect;}ctx.drawImage(design,zone.zoneX+side.positionX*zone.zoneW-width/2,zone.zoneY+side.positionY*zone.zoneH-height/2,width,height);try{canvas.toDataURL('image/png');}catch(exportError){console.error('PrintStich konfigurators: canvas preview eksportēšana neizdevās.',exportError);}}catch(previewError){console.error('PrintStich konfigurators: gala preview neizdevās.',previewError);drawFallbackCanvas(ctx,canvas);}}
  ...
  function syncAttachment(){const side=currentSide();if(!side.file)return;if(typeof DataTransfer==='undefined'){console.warn('PrintStich konfigurators: DataTransfer nav pieejams šajā pārlūkā; oriģinālais fails var netikt pievienots formai.');return;}const transfer=new DataTransfer();transfer.items.add(side.file);els.designInput.files=transfer.files;els.attachmentSlot.appendChild(els.designInput);}
