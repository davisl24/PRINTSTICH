(() => {
  ...
  const els = {
    ...
    uploadZone: $('[data-upload-zone]'), designInput: $('[data-design-input]'), fileName: $('[data-file-name]'),
  ...
  // Vecais JPG mockaps vairs netiek izmantots. Paslēpjam to uzreiz, lai pārlūks
  // SVG ielādes laikā nerādītu nogrieztu alt tekstu (piem., "Ba" no "Balta").
  if (els.shirt) {
  ...
  const createSideState = () => ({ file: null, url: '', image: null, naturalWidth: 0, naturalHeight: 0, positionX: .5, positionY: .5, scale: .5, preset: 'center' });

  const state = {
    step: 1, color: 'balts', size: '', side: 'prieksa', svgLoaded: false, svgRoot: null,
  ...
  function ensureExtraUi() {
    ...
    if (els.form) {
      [['drukas_izmers_mm','data-form-print-mm'],['dizaina_pozicija','data-form-design-position'],['faila_izsirtspeja','data-form-resolution']].forEach(([name,attr]) => {
        if ($(`[${attr}]`, els.form)) return;
        const input = document.createElement('input'); input.type = 'hidden'; input.name = name; input.setAttribute(attr, ''); els.form.appendChild(input);
      });
    }
  ...
  function updateSvgColor() {
    ...
    // Saglabā sākotnējo stroke, bet nodrošina kontrastu visām redzamajām krekla līnijām.
  ...
  function designDimensions(side=currentSide()) {
    ...
    const aspect=side.naturalWidth/side.naturalHeight,maxWidth=area.width*side.scale,maxHeight=area.height*side.scale; let width=maxWidth,height=width/aspect;
    if(height>maxHeight){height=maxHeight;width=height*aspect;} return {area,width,height};
  }
  function printMetrics(side=currentSide()) { const d=designDimensions(side),mmArea=physicalArea(); if(!d||!mmArea)return null; const widthMm=d.width/d.area.width*mmArea.w,heightMm=d.height/d.area.height*mmArea.h,dpi=widthMm>0?side.naturalWidth/widthMm*25.4:0; return {widthMm,heightMm,dpi,widthRounded:Math.round(widthMm),heightRounded:Math.round(heightMm),dpiRounded:Math.round(dpi)}; }
  ...
  function drawDesignCanvas(){const canvas=els.designCanvas,side=currentSide(),rect=els.printArea.getBoundingClientRect();if(!canvas||!rect.width||!rect.height)return;const dpr=Math.min(window.devicePixelRatio||1,2),pixelWidth=Math.max(1,Math.round(rect.width*dpr)),pixelHeight=Math.max(1,Math.round(rect.height*dpr));if(canvas.width!==pixelWidth)canvas.width=pixelWidth;if(canvas.height!==pixelHeight)canvas.height=pixelHeight;const ctx=canvas.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,rect.width,rect.height);if(!side.image)return;const dimensions=designDimensions(side);if(!dimensions)return;ctx.drawImage(side.image,side.positionX*rect.width-dimensions.width/2,side.positionY*rect.height-dimensions.height/2,dimensions.width,dimensions.height);}
  function renderDesign(){updatePrintArea();const side=currentSide();if(!side.url||!side.image){drawDesignCanvas();updateMeasurementUi();updatePresetState();return;}constrainPosition(side);els.scale.value=String(Math.round(side.scale*100));els.scale.max=String(Math.floor(maxAllowedScale(side)*100));drawDesignCanvas();updateMeasurementUi();updatePresetState();}
  ...
  function updateNavigation(){if(els.prev)els.prev.disabled=state.step===1;if(els.next){els.next.hidden=state.step===4;els.next.disabled=!stepComplete(state.step);}}
  ...
  function loadFile(file){const message=validateFile(file);if(message){error('file',message);return;}error('file');const side=currentSide();if(side.url)URL.revokeObjectURL(side.url);side.file=file;side.url=URL.createObjectURL(file);side.image=null;side.positionX=.5;side.positionY=.5;side.scale=.5;side.preset='center';const image=new Image();image.onload=()=>{side.image=image;side.naturalWidth=image.naturalWidth;side.naturalHeight=image.naturalHeight;if(els.fileName)els.fileName.textContent=`${sideLabel(state.side)}: ${file.name}`;enforcePrintLimit();showStep(3);renderDesign();updateNavigation();scrollToPreviewAfterUpload();};image.onerror=()=>{side.file=null;side.image=null;URL.revokeObjectURL(side.url);side.url='';error('file','Attēlu neizdevās nolasīt. Izvēlies citu failu.');renderDesign();updateNavigation();};image.src=side.url;}
  ...
  let drag=null;
  els.designCanvas.addEventListener('pointerdown',event=>{if(!currentSide().url||state.step!==3)return;event.preventDefault();els.designCanvas.setPointerCapture(event.pointerId);els.designCanvas.style.cursor='grabbing';drag={pointerId:event.pointerId};currentSide().preset='';updatePresetState();});
  ...
  async function drawFinalPreview(){const canvas=els.finalPreview;if(!canvas)return;const ctx=canvas.getContext('2d'),side=currentSide(),zone=product.drukasZona[state.side];canvas.width=900;canvas.height=1050;ctx.clearRect(0,0,canvas.width,canvas.height);try{const shirtImage=await svgToImage();ctx.drawImage(shirtImage,0,0,canvas.width,canvas.height);if(side.image&&zone){const zoneX=zone.x*canvas.width,zoneY=zone.y*canvas.height,zoneW=zone.w*canvas.width,zoneH=zone.h*canvas.height,aspect=side.naturalWidth/side.naturalHeight;let width=zoneW*side.scale,height=width/aspect;if(height>zoneH*side.scale){height=zoneH*side.scale;width=height*aspect;}ctx.drawImage(side.image,zoneX+side.positionX*zoneW-width/2,zoneY+side.positionY*zoneH-height/2,width,height);}canvas.toDataURL('image/png');}catch(previewError){console.error('PrintStich konfigurators: priekšskatījumu neizdevās izveidot no SVG.',previewError);ctx.fillStyle='#f7f7f5';ctx.fillRect(0,0,canvas.width,canvas.height);}}

  function syncFormData(){const side=currentSide(),metrics=printMetrics(),color=colorById(state.color),colorInput=$('[data-form-color]'),sizeInput=$('[data-form-size]'),sideInput=$('[data-form-side]'),xInput=$('[data-form-position-x]'),yInput=$('[data-form-position-y]'),scaleInput=$('[data-form-scale]');if(colorInput)colorInput.value=color.nosaukums;if(sizeInput)sizeInput.value=state.size;if(sideInput)sideInput.value=sideLabel(state.side);if(xInput)xInput.value=side.positionX.toFixed(4);if(yInput)yInput.value=side.positionY.toFixed(4);if(scaleInput)scaleInput.value=side.scale.toFixed(4);const mm=$('[data-form-print-mm]'),pos=$('[data-form-design-position]'),res=$('[data-form-resolution]');if(mm)mm.value=metrics?`${metrics.widthRounded} x ${metrics.heightRounded} mm`:'';if(pos)pos.value=`x: ${Math.round(side.positionX*100)}%, y: ${Math.round(side.positionY*100)}%`;if(res)res.value=metrics?`${side.naturalWidth} x ${side.naturalHeight} px, ${metrics.dpiRounded} DPI`:'';}
  function syncAttachment(){const side=currentSide();if(!side.file||!els.designInput||!els.attachmentSlot)return;if(typeof DataTransfer==='undefined'){console.warn('PrintStich konfigurators: DataTransfer nav pieejams šajā pārlūkā; oriģinālais fails var netikt pievienots formai.');return;}const transfer=new DataTransfer();transfer.items.add(side.file);els.designInput.files=transfer.files;els.attachmentSlot.appendChild(els.designInput);}
  ...
  if(els.form)els.form.addEventListener('submit',event=>{error('form');const name=$('[data-customer-name]')?.value.trim()||'',contact=$('[data-customer-contact]')?.value.trim()||'';if(!state.size){event.preventDefault();error('form','Izvēlies krekla izmēru.');return;}if(!currentSide().file){event.preventDefault();error('form','Pievieno dizaina failu.');return;}if(!name){event.preventDefault();error('form','Ievadi savu vārdu.');return;}if(!contact){event.preventDefault();error('form','Ievadi telefonu vai e-pastu.');return;}syncFormData();syncAttachment();});
